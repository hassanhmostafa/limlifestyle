  ...
  return [...readings]
    ...
    .map((r) => ({
      ...
      score: (() => {
        ...
        if (r.sbp != null && r.dbp != null) {
          const s = r.sbp <= 120 && r.dbp <= 80 ? 100
            : r.sbp <= 130 ? 80 : r.sbp <= 140 ? 60 : r.sbp <= 160 ? 40 : 20;
          total += s; count++;
        }
        if (r.hr != null) {
          total += (r.hr >= 60 && r.hr <= 100 ? 100 : r.hr >= 50 && r.hr <= 110 ? 70 : 40);
          count++;
        }
        if (r.bmi != null) {
...
export default function HealthDashboard() {
  ...
  const handleSubmit = () => {
    ...
    logMutation.mutate({
      ...
      sbp: form.bloodPressureSystolic ? parseInt(form.bloodPressureSystolic) : undefined,
      dbp: form.bloodPressureDiastolic ? parseInt(form.bloodPressureDiastolic) : undefined,
      hr: form.heartRate ? parseInt(form.heartRate) : undefined,
    ...
    try {
      ...
      if (latestReading) {
        ...
        autoTable(doc, {
          ...
          body: [
            ["Blood Pressure", latestReading.sbp && latestReading.dbp ? `${latestReading.sbp}/${latestReading.dbp}` : "—", "mmHg"],
            ["Heart Rate", latestReading.hr?.toString() ?? "—", "bpm"],
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text(
          `Generated by LIM · Page ${i} of ${pageCount} · ${now}`,
          14,
          doc.internal.pageSize.height - 8
        );
      }

      doc.save(`LIM_HealthReport_${name.replace(/\s+/g, "_")}_${new Date().toISOString().slice(0, 10)}.pdf`);
    } catch (err) {
      console.error(err);
