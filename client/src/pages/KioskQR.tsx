      ...
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-emerald-50 via-white to-teal-50 p-4">
        <Card className="w-full max-w-sm border-0 shadow-lg">
          <CardContent className="space-y-4 pt-8 pb-8 text-center">
            ...
            <a href={getLoginUrl()}><Button className="w-full bg-[#dff45a] text-[#073f35] hover:bg-[#cde944]">{isAr ? "تسجيل الدخول" : "Sign In"}</Button></a>
          </CardContent>
        </Card>
      </div>
    );
      ...
      <div className="w-full max-w-sm space-y-4">
        <div className="text-center">
          <div className="mb-1 inline-flex items-center gap-2">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#075847]"><Heart className="h-5 w-5 text-white" /></div>
          ...
          <CardContent className="space-y-5 pt-6 pb-6 text-center">
            ...
            {generateQR.isPending ? (
              ...
              <div className="flex min-h-52 flex-col items-center justify-center space-y-3 rounded-xl bg-amber-50 p-5">
                ...
                <Button onClick={() => navigate("/profile")} className="bg-[#dff45a] text-[#073f35] hover:bg-[#cde944]">{isAr ? "فتح الملف الشخصي" : "Open Profile"}</Button>
              </div>
            ) : machineUserId ? (
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-center gap-2 rounded-xl bg-gray-50 p-2">
                  <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-[#075847]"><CheckCircle2 className="h-4 w-4 text-white" /></div>
