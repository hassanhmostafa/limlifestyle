...
## 5. رحلة الفعاليات الموحدة داخل LIM
...
يسجّل المشارك الدخول بحساب LIM ورقم جواله، ثم يحفظ نموذج الفعالية ويعرض QR يحتوي رقم الجوال المحلي `05XXXXXXXX`. يمسح X18 الرمز، ويرسل القياس إلى **نقطة LIM نفسها**:

```text
https://limlifestyle.com/api/kiosk/data?apiKey=YOUR_LIM_UPLOAD_KEY
```

لا يوجد رابط رفع آخر لتطبيق الفعاليات، ولا يحتفظ تطبيق المتصفح بمفتاح الرفع. تحفظ إجابات الفعالية منفصلة في `event_participant_sessions`، بينما تبقى نتيجة X18 الفعلية الوحيدة في `health_readings`. عند وصول تقرير X18، تُربط جلسة الفعالية بالحساب نفسه وتظهر النتيجة في `/events` و**My Health** من السجل ذاته.
