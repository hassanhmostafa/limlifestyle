...
try {
  ...
  if (process.env.EVENTS_OTP_ENABLED === 'true') {
    if (!process.env.AUTHENTICA_API_KEY?.trim()) throw new Error('AUTHENTICA_API_KEY is required when EVENTS_OTP_ENABLED=true');
    await connection.query('SELECT phoneHash, tokenHash, expiresAt, attempts, state FROM event_otp_challenges LIMIT 0');
  ...
  console.error('Check DATABASE_URL targets the deployed database and review migrations 0011/0012/0013/0014. Apply pending migrations with pnpm exec drizzle-kit migrate, then rerun pnpm db:check-events. Do not mark readiness as successful until this passes.');
  process.exitCode = 1;
